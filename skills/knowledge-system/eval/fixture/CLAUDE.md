...
A synthetic monorepo used to evaluate the repo-knowledge system. Two packages: `apps/web`
(order UI) and `libs/auth` (auth adapter). The block below is the knowledge-system contract
under test — it is the v1 contract verbatim (`/contract/knowledge-contract.md`).
